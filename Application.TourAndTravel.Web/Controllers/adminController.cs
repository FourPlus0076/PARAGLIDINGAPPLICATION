...
namespace Application.TourAndTravel.Web.Controllers
{
    ...
    public class adminController : Controller
    {
        ...
        [HttpPost]
        public ActionResult PackageTypeByAdmin(ParaglidingDetails paraglidingDetails, HttpPostedFileBase Image)
        {
            string userName = GetLoggedInUser();
            if (!string.IsNullOrEmpty(userName))
            {
                string ImageName = string.Empty;
                if (paraglidingDetails.PackageTypeID == 0)
                {
                    ImageName = GetImage(Image);
                    if (ImageName != null)
                    {
                        paraglidingDetails.Image = ImageName;
                        paraglidingDetails.CreatedBy = userName;
                        paraglidingDetails.ModifiedBy = "";
                        int i = _packageRepositories.PackageTypeByAdminsave(paraglidingDetails);
                ...
                else
                {
                    ImageName = GetImage(Image);
                    paraglidingDetails.Image = ImageName;
                    paraglidingDetails.ModifiedBy = userName;
                    int i = _packageRepositories.UpdatePackageTypeDataById(paraglidingDetails, out _fileName);
